...
function CategoriesPage() {
      ...
      .then(async ({ data }) => {
        ...
        try {
          const salesRes = await api.get(
            "/relatorios/jogos-mais-vendidos?top=100",
          );
          ...
          console.info(
            "Aviso: Dados de mais vendidos não puderam ser carregados.",
            err,
          );
        }
  ...
  const filteredAndSortedGames = useMemo(() => {
    ...
    if (sortBy === "priceAsc") {
      ...
      result.sort(
        (a, b) =>
          (avgRatings[b.id]?.media || 0) - (avgRatings[a.id]?.media || 0),
      );
  ...
  return (
    <div className="cat-page">
      <div className="cat-page-topbar">
        <div
          className="cat-page-topbar-inner"
          style={{
            display: "flex",
              ...
              {selectedCategory ? (
                <>
                  <span className="material-symbols-outlined">
                    {ICON_MAP[selectedCategory] || "sports_esports"}
                  </span>{" "}
                  {selectedCategory}
                </>
              ) : (
        ...
        <main className="cat-main">
          {filteredAndSortedGames.length === 0 ? (
            <div className="cat-empty">
              ...
              <button
                className="cat-clear-btn cat-clear-btn--center"
              ...
              {filteredAndSortedGames.map((jogo) => {
                ...
                return (
                  <div key={jogo.id} className="cat-game-card">
                    ...
                    >
                      <span className="cat-card-thumb-icon material-symbols-outlined">
                        sports_esports
                      </span>
                      ...
                      <div className="cat-card-rating">
                        <div className="cat-card-stars">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <span
                              key={s}
                              className={`material-symbols-outlined cat-star ${s <= Math.round(ratingMedia) ? "filled" : ""}`}
                            >
                              star
                            </span>
                          ))}
