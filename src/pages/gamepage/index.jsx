  ...
  return (
    <div className="star-display" style={{ fontSize: size }}>
      {[1, 2, 3, 4, 5].map((s) => (
        <span
          key={s}
          className={`material-symbols-outlined star ${s <= Math.round(rating || 0) ? "filled" : ""}`}
        >
          star
        </span>
      ))}
...
function GamePage() {
  ...
  const fetchReviews = useCallback(
    async (jogoId, token) => {
      try {
        const res = await api.get(`/avaliacoes/media/${jogoId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        console.info(
          "Aviso: Nenhuma avaliação encontrada para este jogo ou acesso negado.",
          err.message,
        );
      }
    },
  ...
  useEffect(() => {
    ...
    api
      .get("/public/jogos")
      .then(async (response) => {
        const allGames = Array.isArray(response.data)
          ? response.data
          : response.data.jogos || [];
        const foundGame = allGames.find(
          (g) => g.nome === decodeURIComponent(id),
        );
        ...
        if (foundGame) {
          ...
          if (token) {
            try {
              ...
              const authGame = authGamesList.find(
                (g) => g.nome === foundGame.nome,
              );
              ...
              if (authGame?.id) {
                ...
                const wishGames = Array.isArray(wishRes.data)
                  ? wishRes.data
                  : [];
                ...
                try {
                  const myGamesRes = await api.get("/usuarios/my/games", {
                    headers: { Authorization: `Bearer ${token}` },
                  });
                  const myGames = Array.isArray(myGamesRes.data)
                    ? myGamesRes.data
                    : [];

                  const comprados = myGames.filter(
                    (item) =>
                      item.chaveAtivacao && item.chaveAtivacao.trim() !== "",
                  );

                  if (
                    comprados.some((item) => item.jogo?.id === gameWithId.id)
                  ) {
                    setIsOwned(true);
                  }
                } catch (err) {
                  console.warn(
                    "Aviso: Não foi possível verificar a biblioteca do usuário:",
                    err.message,
                  );
                }

                try {
                  const myReviewRes = await api.get(
                    `/avaliacoes?jogoId=${gameWithId.id}`,
                    { headers: { Authorization: `Bearer ${token}` } },
                  );
                  ...
                  console.info(
                    "Formulário liberado: Avaliação prévia do usuário não encontrada.",
                    err.message,
                  );
                  setUserReview(null);
                  setUserRating(0);
              ...
              console.warn(
                "Acesso à listagem restrita de jogos indisponível:",
                err.message,
              );
            }
          }
          ...
          const cat = foundGame.categoria
            ? String(foundGame.categoria).trim().toLowerCase()
            : "";
      ...
      .catch((err) => {
        console.error(
          "Falha crítica ao buscar o catálogo de jogos:",
          err.message,
        );
        setLoading(false);
      });
  ...
  const handleSubmitReview = async () => {
    if (!userRating) {
      setReviewMsg({
        type: "error",
        text: "Selecione uma nota antes de publicar.",
      });
      return;
    }
    const token = localStorage.getItem("token");
    ...
    if (!finalGameId) {
      try {
        const authRes = await api.get("/jogos", {
          headers: { Authorization: `Bearer ${token}` },
        });
        const jogosList = Array.isArray(authRes.data)
          ? authRes.data
          : authRes.data.jogos || [];
        ...
        if (match?.id) {
          ...
          setReviewMsg({
            type: "error",
            text: "Não foi possível identificar o jogo. Tente recarregar a página.",
          });
          return;
        }
      } catch (err) {
        setReviewMsg({
          type: "error",
          text: `Erro de mapeamento de jogo: ${err.message}`,
        });
        return;
      }
    }
    ...
    try {
      if (userReview) {
        await api.put(
          "/avaliacoes",
          { jogoId: finalGameId, nota: userRating, comentario: reviewText },
          { headers: { Authorization: `Bearer ${token}` } },
        );
        ...
        setReviewMsg({
          type: "success",
          text: "Avaliação publicada com sucesso!",
        });
      }
  ...
  const handleAddToCart = async () => {
    ...
    if (!finalGameId) {
      try {
        ...
        alert(
          `Erro ao mapear identificador: ${err.response?.status || err.message}`,
        );
        return;
      }
    }
  ...
  const handleBuy = async () => {
    ...
    if (!finalGameId) {
      try {
        ...
        alert(
          `Erro de comunicação na pré-compra: ${err.response?.status || err.message}`,
        );
        return;
      }
    }
  ...
  const handleWishlist = async () => {
    ...
    if (!finalGameId) {
      try {
        ...
        console.error(
          "Resgate de ID para lista de desejos falhou:",
          err.message,
        );
        return;
      }
  ...
  if (loading)
    return (
      <div className="game-page-loading">Carregando informações do jogo...</div>
    );
  if (!game)
    return (
        ...
        <section className="game-hero">
          ...
          <aside className="game-sidebar">
            <div className="game-card">
              <div className="game-cover material-symbols-outlined">
                sports_esports
              </div>
              ...
              {isOwned ? (
                <>
                  ...
                  <div className="btn-owned-badge">
                    <span className="material-symbols-outlined">check</span> Na
                    sua biblioteca
                  </div>
                </>
              ) : (
        ...
        {isOwned && (
          <section className="section" id="avaliacoes">
            <h2 className="section-title">
              {userReview ? "Sua avaliação" : "Avaliar este jogo"}
            </h2>
            <div className="section-card">
              <div className="review-form">
                <div className="user-avatar-lg">
                  {getInitialsFromName(getUserName())}
                </div>
                <div className="review-form-content">
                  ...
                  {reviewMsg && (
                    <p className={`review-msg review-msg--${reviewMsg.type}`}>
                      {reviewMsg.text}
                    </p>
                  )}
                </div>
        ...
        <section className="section">
          ...
          <div className="section-card">
            <p className="about-text">
              {game.descricao ||
                "Nenhuma descrição detalhada fornecida para este título ainda."}
            </p>
          </div>
        </section>
          ...
          <section className="section">
            ...
            <div className="similar-games">
              {similarGames.map((similar) => (
                <Link
                  to={`/game/${encodeURIComponent(similar.nome)}`}
                  key={similar.nome}
            ...
            <div className="reviews-list">
              {reviews.map((review) => {
                ...
                const initials = name
                  ...
                  : String(review.fkUsuario).substring(0, 2).toUpperCase();
