...
function Navbar() {
      ...
      api
        ...
        .then((res) => {
          if (res.data.message === "Carrinho vazio." || !res.data.carrinho) {
            ...
            const count = res.data.carrinho.itens
              ? res.data.carrinho.itens.length
              : 0;
            setCartCount(count);
  ...
  const processToken = useCallback((token) => {
    try {
      ...
      setIsLoggedIn(true);
      setIsAdmin(
        decodedPayload.perfil === "Administrador" ||
          decodedPayload.perfil === "Admin",
      );
    } catch (error) {
      console.error(error);
      handleLogout();
      ...
      if (!welcomeShown) {
        window.dispatchEvent(
          new CustomEvent("notify", {
            detail: {
              text: `Bem-vindo (a) de volta, ${user.nome}!`,
              link: "/",
            },
          }),
        );
    ...
    <nav className="navbar">
      <div className="navbar-container">
          ...
          {isSearchOpen && searchResults.length > 0 && (
            <div className="search-dropdown">
              {searchResults.map((jogo) => (
                ...
                >
                  <span className="search-item-icon material-symbols-outlined">
                    sports_esports
                  </span>
        ...
        <div className={`navbar-menu ${isMobileMenuOpen ? "active" : ""}`}>
          ...
          <Link
            to="/categorias"
            className={`navbar-item ${location.pathname === "/categorias" ? "active" : ""}`}
            onClick={() => setIsMobileMenuOpen(false)}
          >
            Categorias
          </Link>
        ...
        {isLoggedIn ? (
          <>
            ...
            <div className="user-profile" ref={menuRef}>
              ...
              {isMenuOpen && (
                <div
                  className={`user-dropdown ${isMenuOpen ? "active" : ""}`}
                  id="user-dropdown"
                >
