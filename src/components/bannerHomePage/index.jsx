  ...
  return (
    <div className="banner-stars-container">
      {[1, 2, 3, 4, 5].map((s) => (
        <span
          key={s}
          className={`material-symbols-outlined banner-star ${s <= Math.round(rating) ? "filled" : ""}`}
        >
          star
        </span>
      ))}
...
function Banner() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setActiveIndex((v) => (v === games.length - 1 ? 0 : v + 1));
    }, 4000);
      ...
      <div className="banner-container">
        <div className="banner-info">
          <div className="banner-label">
            ...
            {currentGame.empresa_nome || "DESTAQUE"}
          </div>
          ...
          <div className="banner-btns">
            {isOwned ? (
              <button
                className="btn-cart btn-cart--owned"
            ...
            <button
              ...
              onClick={() =>
                navigate(`/game/${encodeURIComponent(currentGame.nome)}`)
              }
            >
              Ver detalhes
            </button>
          ...
          <div className="banner-card-rating-mobile">
            <StarRating rating={ratingObj.media} />
            {ratingObj.media > 0 ? (
              <>
                <span className="rating-value">
                  {ratingObj.media.toFixed(1)}
                </span>
                <span className="rating-count">({ratingObj.total})</span>
              </>
            ) : (
